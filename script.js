...
function calculateBollingerBands(data, timePeriod) {
    if(!data || !data["Time Series (Daily)"]) {
         console.error("Errore: i dati necessari per le bande di Bollinger non sono stati restituiti dall'API");
           return null;
    }
   const dailyPrices = Object.values(data["Time Series (Daily)"]).map(item => parseFloat(item["4. close"])).reverse();
    if (dailyPrices.length < timePeriod) {
        console.warn("Avviso: Dati insufficienti per calcolare le bande di Bollinger.");
       return null;
    }
...
function calculateMACD(data) {
   if(!data || !data["Time Series (Daily)"]) {
    console.error("Errore: i dati necessari per il MACD non sono stati restituiti dall'API");
        return null;
    }
 const dailyPrices = Object.values(data["Time Series (Daily)"]).map(item => parseFloat(item["4. close"])).reverse();
    if (dailyPrices.length < 26) {
        console.warn("Avviso: Dati insufficienti per calcolare il MACD.");
         return null;
    }
...
async function aggiornaGrafico(data, signal) {
    ...
    if (!chartContainer) {
        console.error("Errore: Elemento con ID 'chartContainer' non trovato.");
        return;
   }
  if (chart) {
